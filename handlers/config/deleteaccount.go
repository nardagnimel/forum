package handlers

import (
	"forum/email_utils"
	"net/http"
	"text/template"
)

// DeleteAccountHandler traite la demande de suppression de compte de l'utilisateur
func DeleteAccountHandler(w http.ResponseWriter, r *http.Request) {
	// Vérifier que l'utilisateur est authentifié
	userID := email_utils.GetUserID(r)
	if userID == 0 {
		http.Error(w, "Vous devez être connecté pour supprimer votre compte", http.StatusUnauthorized)
		return
	}

	// Afficher la page de confirmation de suppression de compte
	tmpl := template.Must(template.ParseFiles("templates/confirm_delete_account.html"))
	tmpl.Execute(w, nil)
}
